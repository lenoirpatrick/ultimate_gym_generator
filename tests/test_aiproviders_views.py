...
@pytest.mark.django_db
def test_la_page_liste_tous_les_fournisseurs_du_registre(logged_client):
    content = logged_client.get(reverse("aiproviders:list")).content.decode()
...
@pytest.mark.django_db
def test_le_secret_n_est_jamais_rendu_en_clair(logged_client):
    ProviderCredential.objects.create(provider="anthropic", secret=SECRET)

    listing = logged_client.get(reverse("aiproviders:list")).content.decode()
    form = logged_client.get(reverse("aiproviders:edit", args=["anthropic"])).content.decode()
...
@pytest.mark.django_db
def test_un_champ_secret_vide_conserve_la_cle_enregistree(logged_client):
    ProviderCredential.objects.create(provider="anthropic", secret=SECRET)
...
@pytest.mark.django_db
def test_un_fournisseur_inconnu_renvoie_404(logged_client):
    assert logged_client.get(reverse("aiproviders:edit", args=["inexistant"])).status_code == 404


@pytest.mark.django_db
def test_le_test_de_connexion_rend_le_resultat_de_l_adaptateur(logged_client, monkeypatch):
    ProviderCredential.objects.create(provider="anthropic", secret=SECRET)
    monkeypatch.setattr(
    ...
    response = logged_client.post(reverse("aiproviders:test", args=["anthropic"]))
...
@pytest.mark.django_db
def test_le_test_de_connexion_signale_un_fournisseur_non_configure(logged_client):
    response = logged_client.post(reverse("aiproviders:test", args=["mistral"]))
